...
export const TOKEN_REF: CredentialRef = credentialRef('CODEX_OAUTH_TOKEN')

/** Resolve the auth-file path: config wins, then `CODEX_AUTH_FILE`, then the default. */
export function resolveAuthFile(authFile: string | undefined): string {
...
export function parseCodexAuth(text: string): CodexAuth {
  let parsed: unknown
  try {
  ...
  const tokens = (parsed as { tokens?: { access_token?: unknown } | null } | null)?.tokens
  const accessToken = typeof tokens?.access_token === 'string' ? tokens.access_token : undefined
  const maybeLastRefresh = (parsed as { last_refresh?: unknown } | null)?.last_refresh
...
export function apply(ctx: Context, config: Config = {}): () => void {
  ...
  async function clearRef(): Promise<void> {
    ...
    try {
      ...
      ctx.logger.warn('codex-oauth: could not clear %s: %s', TOKEN_REF, (error as Error).message)
    }
  }
  ...
  async function sync(): Promise<void> {
    ...
    try {
      ...
      ctx.logger.warn('codex-oauth: could not resolve %s: %s', TOKEN_REF, (error as Error).message)
      return
    }
    ...
    if (auth.expiresAt !== undefined) {
      ...
      if (secondsLeft <= 0) {
        ctx.logger.error(
          'codex-oauth: %s expired %s ago; run `codex login` or use the Codex CLI to refresh the session',
          TOKEN_REF,
          humanizeDuration(-secondsLeft),
        )
        await clearRef()
        return
      }
      if (secondsLeft < skewSeconds) {
        ctx.logger.warn(
          'codex-oauth: %s expires in %s; run `codex login` or use the Codex CLI to refresh the session',
          TOKEN_REF,
          humanizeDuration(secondsLeft),
        )
      } else if (changed) {
    ...
    try {
      ...
      ctx.logger.warn('codex-oauth: could not store %s: %s', TOKEN_REF, (error as Error).message)
    }
  }
    ...
    const startWatcher = (): void => {
      ...
      try {
        ...
        watcher = fsWatch(dir, { persistent: true }, (_event, filename) => {
          if (closed) return
          if (filename === null || basename(String(filename)) === basenameOfAuth) scheduleSync()
        })
